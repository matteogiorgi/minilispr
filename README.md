...
- **Translator** (`to_r`) — purely syntactic S-expression $\to$ R `call`/`name` conversion. No Lisp code is executed at this stage; closures only come into existence once the resulting call is `eval()`&rsquo;d, so they capture whatever environment is active at that point with nothing threaded around by hand.
